...
$(function(){
    ...
    function formatOpt(value, colObjet, colValue){
    	var a1 = "<a class='gridOper' href=\"javascript:cancelZjhbpcMx("+colValue.ID+")\" ><img src=\"images/grid_del.png\">删除</a> "; 
		return a1;
	}
...
function reloadGrid(){
	myAjax({ url: "czj/getZjhbpcmxList", 
		data: {pcId: pcId},
		success: function(rs){
			var list = rs.data;
...
function addPcmx(){
	openDialog_L("czj/zjhbpcSelectMx?id="+pcId, '选择添加划拨明细');
}
function cancelZjhbpcMx(id){
	myAjax({ url: "czj/cancelZjhbpcMx", 
		data: {id: id, ZJHBPC_ID: pcId},
		success: function(rs){
			showSuccessMsg("删除成功");
			reloadGrid();
		}
	...
	<div id="main" class="main">
		<div class="whiteBox actionArea">
			<div class="flexigrid">
				...
				<form id="validForm" action="czj/saveZjhbpc" method="post">
				<input type="hidden" name="ID" value="${data.ID}">
				    ...
				    <input type="button" class="button" value="添加明细" onclick="addPcmx()"/>
				    <input type="button" class="button redButton" value="保存" onclick="doCommonSubmit()"/>
				</form>
			    </div>
